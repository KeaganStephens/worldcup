...
echo "$($PSQL "SELECT to_char(AVG(winner_goals + opponent_goals), 'FM999999999999990.0000000000000000') FROM games")"
...
echo "$($PSQL "SELECT name FROM teams INNER JOIN games ON teams.team_id = games.winner_id WHERE year = 2018 GROUP BY teams.name ORDER BY COUNT(winner_id) DESC LIMIT 1")"
